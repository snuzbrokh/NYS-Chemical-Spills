...
```{r}
offenders = storage %>% 
  inner_join(spills, by=c('Facility.Name','Locality','County','DEC.Region','Material')) %>% 
  filter(Quantity.y > 0) %>% 
  select(DEC.Region,Facility.Name,Material,Locality,County,Tank.Location, Quantity.x,Quantity.y, `Case Lag`,Lat.x,Lon.x,`Spill Date`) %>% 
  filter(`Case Lag` > 0 & `Case Lag` < 2e3) %>% 
  group_by(Tank.Location, `Spill Date`,DEC.Region, County,Locality,Facility.Name,Material) %>% 
  #filter(`Case Lag` > 0 & `Case Lag` < 1e3) %>% 
  summarise(spill_volume = sum(Quantity.y), num_spills = n(), case.lag = median(`Case Lag`)) %>%
...
```{r}
offenders_num = offenders %>% select(spill_volume, num_spills, case.lag)
cor(offenders_num, method = "pearson" )
...
```{r}
offenders %>% 
  ggplot(aes(y=case.lag,x=log(spill_volume))) + geom_point() + facet_grid(Tank.Location~.)
```
...
spills %>% 
  group_by(Material.Family) %>% 
  filter(`Case Lag` > 0) %>% 
  ggplot(aes(x = `Case Lag`)) +
  geom_histogram(bins=30) + facet_wrap(.~Material.Family, scales="free")

```

```{r}

topPetrolSpills = spills %>% 
...
spills %>% 
  filter(Material.Family == 'Petroleum') %>% 
  group_by(Material) %>% 
  semi_join(topPetrolSpills, by='Material') %>% 
  filter(`Report Lag` > 0) %>%
  ggplot(aes(x = `Report Lag`)) +
  geom_histogram(bins=30) + facet_wrap(.~Material, scales="free")
```
